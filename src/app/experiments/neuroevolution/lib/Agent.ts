...
import { BoardState, Direction, Position } from '../components/Board/lib/BoardState'
...
export class Agent {
  ...
  fight(otherAgent: Agent): boolean {
    const isConflict =
      this.position[0] === otherAgent.position[0] && this.position[1] === otherAgent.position[1]
  ...
  move(boardState: BoardState): Agent {
    ...
    const nextPosition = boardState.calculateMove(this.position, direction || this.direction)
  ...
  private threatDistance(boardState: BoardState, offset: number): number {
    ...
    let threatDistances = boardState
      ...
      .filter(([, y]) => y === currentY) // on the current row
      .map(([x]) => x - currentX) // distance from current position (- == left, + == right)
