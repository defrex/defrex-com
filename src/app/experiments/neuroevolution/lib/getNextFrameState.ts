import { map, max, maxBy, min, random, range, some, sum, zipObject } from 'lodash'
import { BoardState, ColorPosition, Position } from '../components/Board/lib/BoardState'
...
export function getNextFrameState(state: FrameState): FrameState {
  ...
  const agents = movedAgents.filter((agent) => isNotInPositions(killPositions, agent.position))

  killPositions = killPositions.filter(isNotInPositions.bind(null, map(movedAgents, 'position')))
  ...
  const respawnAgent = maxBy([...agents, state.respawnAgent], (agent) => agent?.lineage)

  while (agents.length < minAgents) {
    agents.push(respawnAgent?.mutate() ?? initAgent(state.gridWidth, state.gridHeight))
  }
  ...
  const history: Array<Record<string | 'move', number>> = [
    ...state.history.slice(-max([fastSampleDuration, slowSampleRate])!),
    {
      ...
      complexityMin: max(agents.map(({ genome }) => genome.nodes.length + genome.edges.length))!,
      complexityMax: min(agents.map(({ genome }) => genome.nodes.length + genome.edges.length))!,
    },
  ]

  const prevDifficulty = state.history.slice(-difficultySmoothing).map((entry) => entry.difficulty)
  ...
  for (const metricSpeed of ['fast', 'slow'] as ['fast', 'slow']) {
    ...
    const sampleDuration = metricSpeed === 'fast' ? fastSampleDuration : slowSampleDuration
    ...
    if (move % sampleRate === 0) {
      for (const metricName of metricNames) {
        ...
        if (lineMetricNames.includes(metricName)) {
          metricValue = {
            move,
            value: sum(history.slice(-lookBack).map((history) => history[metricName])) / lookBack,
          }
        } else {
          metricValue = {
            move,
            min:
              sum(history.slice(-lookBack).map((history) => history[`${metricName}Min`])) /
              lookBack,
            max:
              sum(history.slice(-lookBack).map((history) => history[`${metricName}Max`])) /
              lookBack,
          }
        }

        metrics[metricSpeed][metricName] = [
          ...metrics[metricSpeed][metricName].slice(-(sampleDuration / sampleRate)),
          metricValue,
        ]
      }
    }
...
export function agentPositionColors(agents: Agent[], gridWidth: number): ColorPosition[] {
...
export function advanceKillPositions(boardState: BoardState): Position[] {
  return boardState
    ...
    .filter(([x, y]) => x >= 0 && y >= 0 && x < boardState.gridWidth && y < boardState.gridHeight)
}
